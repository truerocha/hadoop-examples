...
function outputJson($hosts, $services, $program)
{
	...
	$jsonstring .= $temp; 
        foreach ($program as $key => $val) {     
            $temp = '    "' . jsonString($key) . '": "' . jsonString($val) . '"' . (isLast($program, $key) ? '' : ',') . "\n";
            echo $temp;
            $jsonstring .= $temp;
        }
        unset($key, $val);
    ...
    foreach ($hosts as $hostName => $hostArray) {
        $temp = '   "' . jsonString($hostName) . '": {' . "\n";
        echo $temp;
	$jsonstring .= $temp; 
        foreach ($hostArray as $key => $val) {
            $temp = '      "' . jsonString($key) . '": "' . jsonString($val) . '"' . (isLast($hostArray, $key) ? '' : ',') . "\n";
            echo $temp;
	    $jsonstring .= $temp;
        }
        unset($key, $val);
        $temp = '   }' . (isLast($hosts, $hostName) ? '' : ',') . "\n";
        echo $temp;
        $jsonstring .= $temp;
    }
    ...
    foreach ($services as $hostName => $service) {
        ...
        foreach ($service as $serviceDesc => $serviceArray) {
            $temp = '   "' . jsonString($serviceDesc) . '": {' . "\n";
            echo $temp;
            $jsonstring .= $temp;
            foreach ($serviceArray as $key => $val) {
                $temp = '      "' . jsonString($key) . '": "' . jsonString($val) . '"' . (isLast($serviceArray, $key) ? '' : ',') . "\n";
  		echo $temp;
                $jsonstring .= $temp;
            }
            unset($key, $val);
            $temp = '   }' . (isLast($service, $serviceDesc) ? '' : ',') . "\n";
            echo $temp;
	    $jsonstring  .= $temp;
        }
        $temp = '   }' . (isLast($services, $hostName) ? '' : ',') . "\n";
        echo $temp;
        $jsonstring .= $temp;
...
//parsing request and displaying data
function requestdata($json_data){
    if($_GET['req']){
        $result = $json_data;
	$my_array = split(":", $_GET['req']);
        print_r($my_array);
        echo "hello world \n";
